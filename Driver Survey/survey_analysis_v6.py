...
def safe_page(pdf, title):
    """Context manager that catches column-related errors and shows a placeholder page."""
    n_figs_before = set(plt.get_fignums())
    try:
        yield
    except (KeyError, TypeError, ValueError) as e:
        # Close any figures that were opened but not saved
...
with PdfPages(OUTPUT_PDF) as pdf:
    ...
    with safe_page(pdf, 'Page 30 - by cooperation type -- do full-time drivers rate differently?'):
    ...
    with safe_page(pdf, 'Page 31 - by city (top 10) -- which cities have the happiest drivers?'):
    ...
    with safe_page(pdf, 'Page 32 - by driver type -- joint drivers can compare platforms directly.'):
    ...
    with safe_page(pdf, 'Page 34 - SATISFACTION BY AGE GROUP'):
    ...
    with safe_page(pdf, 'Page 35 - SATISFACTION BY ACTIVE TIME (ENGAGEMENT LEVEL)'):
    ...
    with safe_page(pdf, 'Page 36 - SATISFACTION BY OCCUPATION'):
    ...
    with safe_page(pdf, 'Page 45 - Incentive Type distribution from long survey'):
    ...
    with safe_page(pdf, 'Page 46 - Whether drivers received their incentive bonus'):
    ...
    with safe_page(pdf, 'Page 47 - CS contact categories from the long survey perspective'):
