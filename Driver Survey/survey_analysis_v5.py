...
for df in [short, wide, long]:
    ...
    df["yearweek"] = (df["year"] % 100) * 100 + df["weeknumber"].astype(int)
...
if len(dropped_weeks) > 0:
    print(f"Dropping {len(dropped_weeks)} week(s) with <{MIN_WEEK_RESPONSES} responses")
...
print(f"Remaining: {len(short)} short, {len(wide)} wide, {len(long)} long, {len(valid_weeks)} weeks")

short["driver_type"] = np.where(short["tapsi_ride"] == 0, "Snapp Exclusive", "Joint")
wide["driver_type"]  = np.where(wide["tapsi_ride"]  == 0, "Snapp Exclusive", "Joint")
long["driver_type"]  = np.where(long["tapsi_ride"]  == 0, "Snapp Exclusive", "Joint")
...
def plot_sat_by_group(pdf, df, groupcol, title_suffix, figsize=(14, 6),
    ...
    fig, axes = plt.subplots(1, 3, figsize=figsize, facecolor=BG_COLOR, sharey=True)
    fig.suptitle(f"Avg Satisfaction by {title_suffix}", fontsize=15, fontweight="bold", y=0.99)
    x = np.arange(len(groups)); w = 0.35
    for ax, (scol, tcol, label) in zip(axes, SAT_PAIRS):
        grp = df_f.groupby(groupcol).agg(snapp=(scol,"mean"), tapsi=(tcol,"mean")).reindex(groups)
        bars_s = ax.bar(x - w/2, grp["snapp"], w, color=SNAPP_COLOR, label="Snapp")
        bars_t = ax.bar(x + w/2, grp["tapsi"], w, color=TAPSI_COLOR, label="Tapsi")
        ...
        ax.set_title(label, fontsize=11); ax.set_ylim(0, 5.5)
        ax.legend(frameon=False, fontsize=8); style_ax(ax)
...
def plot_long_snapp_vs_tapsi(pdf, long_df, snapp_question, tapsi_question, title, figsize=(14, 6)):
    fig, axes = plt.subplots(1, 2, figsize=figsize, facecolor=BG_COLOR, sharey=True)
    ...
    for ax, q, color, label in [
        ...
        if len(qdata) == 0:
            ax.set_title(f"{label}  (no data)", fontsize=11); style_ax(ax); continue
        answer_counts = qdata["answer"].value_counts().sort_values(ascending=True)
        total = answer_counts.sum()
        ax.barh(answer_counts.index, answer_counts.values, color=color, edgecolor="white")
        for i, (ans, v) in enumerate(answer_counts.items()):
            ax.text(v+0.5, i, f"{v} ({v/total*100:.0f}%)", va="center", fontsize=9)
        ax.set_title(f"{label}  (n={total})", fontsize=11); style_ax(ax)
    axes[0].set_xlabel("Count"); axes[1].set_xlabel("Count")
    save_fig(pdf, fig)
...
with PdfPages(OUTPUT_PDF) as pdf:
    ...
    fig.suptitle("Driver Survey – Key Performance Indicators", fontsize=18, fontweight="bold", y=0.97)
    ax_banner = fig.add_axes([0.05, 0.82, 0.9, 0.1])
    ax_banner.set_xlim(0, 10); ax_banner.set_ylim(0, 1); ax_banner.axis("off")
    ...
    for i, (val, lbl) in enumerate(kpis):
        cx = 1 + i*2
        ax_banner.text(cx, 0.7, val, ha="center", fontsize=20, fontweight="bold", color=ACCENT)
    ...
    for xi, (sv, tv) in enumerate(zip(s_vals, t_vals)):
        ax_sat.text(xi-w/2, sv+0.05, f"{sv:.2f}", ha="center", fontsize=8, fontweight="bold")
        ax_sat.text(xi+w/2, tv+0.05, f"{tv:.2f}", ha="center", fontsize=8, fontweight="bold")
    ax_sat.set_xticks(x); ax_sat.set_xticklabels(cats, fontsize=9, rotation=15, ha="right")
    ax_sat.set_ylim(0, 5.5); ax_sat.set_title("Mean Satisfaction (1–5)", fontsize=11)
    ax_sat.legend(frameon=False, fontsize=9); style_ax(ax_sat)

    ax_nps = fig.add_axes([0.55, 0.52, 0.4, 0.26])
    metrics = ["NPS","Avg Incentive\n(M Rials)","CS Satisfaction\n(Overall)"]
    s_m = [snapp_nps_val, snapp_inc_mean, short["snapp_CS_satisfaction_overall"].mean()]
    t_m = [tapsi_nps_val, tapsi_inc_mean, short["tapsi_CS_satisfaction_overall"].mean()]
    ...
    for xi, (sv, tv) in enumerate(zip(s_m, t_m)):
        ax_nps.text(xi-w/2, max(sv,0)+0.2, f"{sv:.1f}", ha="center", fontsize=8, fontweight="bold")
        ax_nps.text(xi+w/2, max(tv,0)+0.2, f"{tv:.1f}", ha="center", fontsize=8, fontweight="bold")
    ax_nps.set_xticks(x2); ax_nps.set_xticklabels(metrics, fontsize=9)
    ax_nps.axhline(0, color=GREY, linewidth=0.8, linestyle="--")
    ax_nps.set_title("NPS, Incentive & CS Overview", fontsize=11)
    ax_nps.legend(frameon=False, fontsize=9); style_ax(ax_nps)

    ax_demo = fig.add_axes([0.05, 0.28, 0.9, 0.18])
    style_ax(ax_demo); ax_demo.set_xlim(0, 10); ax_demo.set_ylim(0, 1); ax_demo.axis("off")
    ...
    top_job_pct = short["original_job"].value_counts().iloc[0] / len(short) * 100
    ...
    demo_items = [(f"{gender_m:.0f}%","Male Drivers"),(f"{age_u35:.0f}%","Under 35"),
                  (f"{part_time:.0f}%","Part-Time"),(f"{top_job_pct:.0f}%",f"#1 Job:\n{top_job}"),
                  (f"{top_city_pct:.0f}%",f"Top City:\n{top_city}")]
    for i,(val,lbl) in enumerate(demo_items):
        cx = 1+i*2
        ax_demo.text(cx, 0.75, val, ha="center", fontsize=16, fontweight="bold", color=ACCENT2)
        ax_demo.text(cx, 0.1, lbl, ha="center", fontsize=9, color=GREY)
    ax_demo.set_title("Demographics Snapshot", fontsize=11, x=0.5, y=1.0, pad=0)

    ax_ins = fig.add_axes([0.05, 0.04, 0.9, 0.2])
    style_ax(ax_ins); ax_ins.axis("off")
    insights = [
    ...
    ax.bar(week_counts.index.astype(str), week_counts.values, color=ACCENT, edgecolor="white", linewidth=0.5)
    bar_label(ax)
    ax.set_xlabel("Year-Week"); ax.set_ylabel("Responses")
    ax.tick_params(axis="x", rotation=45); style_ax(ax)
    save_fig(pdf, fig)
    ...
    fig.suptitle("Demographics Overview", fontsize=15, fontweight="bold", y=0.97)
    ax = axes[0,0]
    ag = short["age_group"].value_counts()
    ax.barh(ag.index, ag.values, color=[ACCENT,GREY]); ax.set_title("Age Group", fontsize=11)
    for i,v in enumerate(ag.values): ax.text(v+5, i, f"{v} ({v/len(short)*100:.0f}%)", va="center", fontsize=9)
    ax = axes[0,1]
    ed = short["edu"].value_counts().sort_index()
    ax.barh(["High School\nor Below","College+"], ed.values, color=[GREY,ACCENT]); ax.set_title("Education", fontsize=11)
    for i,v in enumerate(ed.values): ax.text(v+5, i, f"{v} ({v/len(short)*100:.0f}%)", va="center", fontsize=9)
    ax = axes[1,0]
    ct = short["cooperation_type"].value_counts()
    ax.barh(ct.index, ct.values, color=[SNAPP_COLOR,TAPSI_COLOR]); ax.set_title("Cooperation Type", fontsize=11)
    for i,v in enumerate(ct.values): ax.text(v+5, i, f"{v} ({v/len(short)*100:.0f}%)", va="center", fontsize=9)
    ax = axes[1,1]
    ms = short["marr_stat"].value_counts().sort_index()
    ax.barh(["Single","Married"], ms.values, color=[GREY,ACCENT]); ax.set_title("Marital Status", fontsize=11)
    for i,v in enumerate(ms.values): ax.text(v+5, i, f"{v} ({v/len(short)*100:.0f}%)", va="center", fontsize=9)
    for row in axes:
        for ax in row: style_ax(ax)
    save_fig(pdf, fig)
    ...
    fig, ax = new_fig("Primary Occupation of Surveyed Drivers (Top 15)", figsize=(12, 7))
    colors = [ACCENT if i<5 else LGREY for i in range(len(job_counts))]
    ax.barh(job_counts.index[::-1], job_counts.values[::-1], color=colors[::-1], edgecolor="white")
    total = len(short)
    for i,v in enumerate(job_counts.values[::-1]):
        ax.text(v+30, i, f"{v:,} ({v/total*100:.1f}%)", va="center", fontsize=9)
    ax.set_xlabel("Response Count"); style_ax(ax)
    save_fig(pdf, fig)

    # ================================================================
    # PAGE 5 – ACTIVE JOINT RATE BY YEARWEEK
    # ================================================================
    weekly_joint = short.groupby("yearweek").agg(total=("active_joint","size"), active=("active_joint","sum"))
    weekly_joint["rate"] = weekly_joint["active"] / weekly_joint["total"] * 100
    fig, ax = new_fig("Active Joint (Tapsi) Rate by Year-Week")
    ax.plot(weekly_joint.index.astype(str), weekly_joint["rate"], marker="o", color=TAPSI_COLOR, linewidth=2.5, markersize=8)
    for idx, row in weekly_joint.iterrows():
        ax.annotate(f"{row['rate']:.0f}%", (str(idx), row["rate"]), textcoords="offset points", xytext=(0,10), ha="center", fontsize=9)
    ax.set_xlabel("Year-Week"); ax.set_ylabel("Active Joint Rate (%)")
    ax.set_ylim(0, 100); ax.tick_params(axis="x", rotation=45); style_ax(ax)
    save_fig(pdf, fig)

    # ================================================================
    # PAGE 6 – AVERAGE RIDE COUNTS
    # ================================================================
    weekly_rides = short.groupby("yearweek").agg(snapp_ride=("snapp_ride","mean"), tapsi_ride=("tapsi_ride","mean"))
    fig, ax = new_fig("Average Weekly Ride Count – Snapp vs Tapsi")
    ax.plot(weekly_rides.index.astype(str), weekly_rides["snapp_ride"], marker="o", color=SNAPP_COLOR, linewidth=2.5, label="Snapp")
    ax.plot(weekly_rides.index.astype(str), weekly_rides["tapsi_ride"], marker="s", color=TAPSI_COLOR, linewidth=2.5, label="Tapsi")
    ax.legend(frameon=False, fontsize=10); ax.set_xlabel("Year-Week"); ax.set_ylabel("Avg Rides")
    ax.tick_params(axis="x", rotation=45); style_ax(ax)
    save_fig(pdf, fig)
    ...
    # ================================================================
    fig, axes = plt.subplots(1, 3, figsize=(14, 5), facecolor=BG_COLOR, sharey=True)
    fig.suptitle("Satisfaction Comparison (1–5 scale): Snapp vs Tapsi", fontsize=15, fontweight="bold", y=0.99)
    for ax, (scol, tcol, label) in zip(axes, SAT_PAIRS):
        snapp_mean = short[scol].dropna().mean(); tapsi_mean = short[tcol].dropna().mean()
        bars = ax.bar(["Snapp","Tapsi"], [snapp_mean, tapsi_mean], color=[SNAPP_COLOR,TAPSI_COLOR], width=0.5, edgecolor="white")
        ax.set_title(label, fontsize=11); ax.set_ylim(0, 5.5); style_ax(ax)
        for bar in bars:
    ...
    # ================================================================
    fig, axes = plt.subplots(1, 2, figsize=(12, 5), facecolor=BG_COLOR, sharey=True)
    fig.suptitle("Overall Satisfaction Distribution (1–5 scale)", fontsize=15, fontweight="bold", y=0.99)
    for ax, col, color, label in [(axes[0],"snapp_overall_satisfaction",SNAPP_COLOR,"Snapp"),
        ...
        ax.bar(counts.index.astype(int).astype(str), counts.values, color=color, edgecolor="white")
        total = counts.sum()
        for x, y in zip(counts.index.astype(int).astype(str), counts.values):
            ax.text(x, y+1, f"{y}\n({y/total*100:.0f}%)", ha="center", fontsize=8)
        ax.set_title(f"{label}  (n={int(total)}, mean={data.mean():.2f})", fontsize=11)
        ax.set_xlabel("Rating"); style_ax(ax)
    ...
    if not nps_weekly["snapp_nps"].isna().all():
        ax.plot(nps_weekly.index.astype(str), nps_weekly["snapp_nps"], marker="o", color=SNAPP_COLOR, linewidth=2.5, label="Snapp")
    if not nps_weekly["tapsi_nps"].isna().all():
        ax.plot(nps_weekly.index.astype(str), nps_weekly["tapsi_nps"], marker="s", color=TAPSI_COLOR, linewidth=2.5, label="Tapsi")
    ax.axhline(0, color=GREY, linestyle="--", linewidth=0.8)
    ax.legend(frameon=False, fontsize=10); ax.set_xlabel("Year-Week"); ax.set_ylabel("NPS")
    ax.tick_params(axis="x", rotation=45); style_ax(ax)
    save_fig(pdf, fig)
    ...
    fig.suptitle("Incentive Category Distribution", fontsize=15, fontweight="bold", y=0.99)
    for ax, col, color, label in [(axes[0],"snapp_incentive_category",SNAPP_COLOR,"Snapp"),
        ...
        total = data.sum()
        for i,v in enumerate(data.values): ax.text(v+2, i, f"{v} ({v/total*100:.0f}%)", va="center", fontsize=9)
        ax.set_title(f"{label}  (n={total})", fontsize=11); style_ax(ax)
    save_fig(pdf, fig)
    ...
    fig, axes = plt.subplots(1, 2, figsize=(14, 6), facecolor=BG_COLOR, sharey=True)
    fig.suptitle("Incentive Type Usage – wide_survey (multi-choice)", fontsize=15, fontweight="bold", y=0.99)
    for ax, (platform, cols) in zip(axes, incentive_types.items()):
        ...
        for i,v in enumerate(values): ax.text(v+5, i, str(int(v)), va="center", fontsize=9)
        ax.set_title(platform, fontsize=12); style_ax(ax)
    axes[0].invert_yaxis()
    save_fig(pdf, fig)
    ...
    fig, axes = plt.subplots(1, 2, figsize=(14, 5), facecolor=BG_COLOR, sharey=True)
    fig.suptitle("Incentive Unsatisfaction Reasons – wide_survey", fontsize=15, fontweight="bold", y=0.99)
    for ax, (platform, cols) in zip(axes, unsat_types.items()):
        labels = list(cols.keys()); values = [wide[c].sum() if c in wide.columns else 0 for c in cols.values()]
        color = PLATFORM_COLORS[platform]; y_pos = range(len(labels))
        ax.barh(y_pos, values, color=color, edgecolor="white")
        ax.set_yticks(y_pos); ax.set_yticklabels(labels)
        for i,v in enumerate(values): ax.text(v+2, i, str(int(v)), va="center", fontsize=9)
        ax.set_title(platform, fontsize=12); style_ax(ax)
    axes[0].invert_yaxis()
    save_fig(pdf, fig)
    ...
    # ================================================================
    weekly_inc = short.groupby("yearweek").agg(snapp=("snapp_incentive","mean"), tapsi=("tapsi_incentive","mean"))
    fig, ax = new_fig("Average Monetary Incentive by Year-Week (Rials)")
    ax.plot(weekly_inc.index.astype(str), weekly_inc["snapp"]/1e6, marker="o", color=SNAPP_COLOR, linewidth=2.5, label="Snapp")
    ax.plot(weekly_inc.index.astype(str), weekly_inc["tapsi"]/1e6, marker="s", color=TAPSI_COLOR, linewidth=2.5, label="Tapsi")
    ax.legend(frameon=False, fontsize=10); ax.set_xlabel("Year-Week"); ax.set_ylabel("Avg Incentive (Million Rials)")
    ax.tick_params(axis="x", rotation=45); style_ax(ax)
    save_fig(pdf, fig)
    ...
    # ================================================================
    fig, axes = plt.subplots(1, 2, figsize=(12, 5), facecolor=BG_COLOR, sharey=True)
    fig.suptitle("Incentive Satisfaction Distribution (1–5 scale)", fontsize=15, fontweight="bold", y=0.99)
    for ax, col, color, label in [(axes[0],"snapp_overall_incentive_satisfaction",SNAPP_COLOR,"Snapp"),
        ...
        data = short[col].dropna(); counts = data.value_counts().sort_index()
        ax.bar(counts.index.astype(int).astype(str), counts.values, color=color, edgecolor="white")
        total = counts.sum()
        for x, y in zip(counts.index.astype(int).astype(str), counts.values):
            ax.text(x, y+1, f"{y}\n({y/total*100:.0f}%)", ha="center", fontsize=8)
    ...
    # ================================================================
    fig, axes = plt.subplots(1, 2, figsize=(12, 5), facecolor=BG_COLOR, sharey=True)
    fig.suptitle("Length of Cooperation Distribution (months)", fontsize=15, fontweight="bold", y=0.99)
    for ax, col, color, label in [(axes[0],"snapp_LOC",SNAPP_COLOR,"Snapp"),
        ...
        ax.text(data.mean()+1, ax.get_ylim()[1]*0.9, f"Mean: {data.mean():.1f}", fontsize=9)
        ax.set_title(f"{label}  (n={len(data)})", fontsize=11); ax.set_xlabel("Months"); style_ax(ax)
    axes[0].set_ylabel("Count")
    save_fig(pdf, fig)
    ...
    tapsi_refusal_cols = [c.replace("Snapp","Tapsi") for c in snapp_refusal_cols]
    snapp_vals = [wide[c].sum() if c in wide.columns else 0 for c in snapp_refusal_cols]
    tapsi_vals = [wide[c].sum() if c in wide.columns else 0 for c in tapsi_refusal_cols]
    fig, ax = new_fig("Ride Refusal Reasons – Snapp vs Tapsi (wide_survey)", figsize=(14, 7))
    y = np.arange(len(refusal_labels)); h = 0.35
    ax.barh(y-h/2, snapp_vals, h, color=SNAPP_COLOR, label="Snapp", edgecolor="white")
    ax.barh(y+h/2, tapsi_vals, h, color=TAPSI_COLOR, label="Tapsi", edgecolor="white")
    ax.set_yticks(y); ax.set_yticklabels(refusal_labels)
    ax.invert_yaxis(); ax.legend(frameon=False, fontsize=10); ax.set_xlabel("Count"); style_ax(ax)
    save_fig(pdf, fig)
    ...
    tapsi_cs = [f"Tapsi Customer Support Category__{l.replace('Location Change','Loc Change')}" for l in cs_labels]
    snapp_cs_vals = [wide[c].sum() if c in wide.columns else 0 for c in snapp_cs]
    tapsi_cs_vals = [wide[c].sum() if c in wide.columns else 0 for c in tapsi_cs]
    fig, ax = new_fig("Customer Support Ticket Categories – wide_survey", figsize=(14, 6))
    y = np.arange(len(cs_labels)); h = 0.35
    ax.barh(y-h/2, snapp_cs_vals, h, color=SNAPP_COLOR, label="Snapp", edgecolor="white")
    ax.barh(y+h/2, tapsi_cs_vals, h, color=TAPSI_COLOR, label="Tapsi", edgecolor="white")
    ax.set_yticks(y); ax.set_yticklabels(cs_labels)
    ax.invert_yaxis(); ax.legend(frameon=False, fontsize=10); ax.set_xlabel("Count"); style_ax(ax)
    save_fig(pdf, fig)
    ...
    stages = ["Navigation Familiarity","Navigation Installed","Navigation Used"]
    stage_labels = ["Familiarity","Installed","Used"]
    fig, axes = plt.subplots(1, 3, figsize=(15, 5), facecolor=BG_COLOR, sharey=True)
    fig.suptitle("Navigation App Adoption Funnel – long_survey", fontsize=15, fontweight="bold", y=0.99)
    for ax, stage, slbl in zip(axes, stages, stage_labels):
        ...
        for i,v in enumerate(vals): ax.text(v+10, i, str(int(v)), va="center", fontsize=9)
        ax.set_title(slbl, fontsize=12); style_ax(ax)
    axes[0].invert_yaxis()
    save_fig(pdf, fig)
    ...
    fig.suptitle("Navigation App Ratings by Drivers (0–10 scale)", fontsize=15, fontweight="bold", y=0.99)
    ax = axes[0]
    ...
    for bar, mean, n in zip(bars, means, ns):
        ax.text(bar.get_x()+bar.get_width()/2, mean+0.1, f"{mean:.2f}\n(n={n:,})", ha="center", fontsize=9, fontweight="bold")
    ax.set_ylim(0, 11); ax.set_ylabel("Mean Rating (0–10)")
    ax.set_title("Mean Recommendation Score", fontsize=11); style_ax(ax)
    ax = axes[1]
    for col, color, label in [("recommendation_neshan","#66BB6A","Neshan"),
        ...
        ax.plot(vals.index, vals.values, marker="o", color=color, linewidth=2, label=f"{label} (n={len(data):,})")
    ax.set_xlabel("Rating (0–10)"); ax.set_ylabel("Count")
    ...
    # ================================================================
    gps_stages = ["No problem","Offer","in route to passenger","Origin to Destination route","All stages"]
    fig, axes = plt.subplots(1, 2, figsize=(12, 5), facecolor=BG_COLOR, sharey=True)
    fig.suptitle("GPS Failure Stage Distribution – Snapp vs Tapsi", fontsize=15, fontweight="bold", y=0.99)
    for ax, col, color, label in [(axes[0],"snapp_gps_stage",SNAPP_COLOR,"Snapp"),
        ...
        for i,v in enumerate(ordered):
            if v>0: ax.text(v+5, i, f"{v} ({v/total*100:.0f}%)", va="center", fontsize=9)
        ax.set_title(f"{label}  (n={total})", fontsize=11); style_ax(ax)
    save_fig(pdf, fig)
    ...
    time_vals = [wide[c].sum() if c in wide.columns else 0 for c in gps_time_cols.values()]
    fig, ax = new_fig("When GPS Glitches Occur – Time of Day (wide_survey)", figsize=(12, 5))
    ...
    ax.set_ylabel("Count"); ax.tick_params(axis="x", rotation=15); style_ax(ax)
    save_fig(pdf, fig)
    ...
    gps_action_vals = [wide[c].sum() if c in wide.columns else 0 for c in gps_action_map.values()]
    fig, ax = new_fig("Driver Actions During GPS Glitch – wide_survey")
    ax.barh(list(gps_action_map.keys()), gps_action_vals, color=ACCENT, edgecolor="white")
    for i,v in enumerate(gps_action_vals): ax.text(v+5, i, str(int(v)), va="center", fontsize=9)
    ax.invert_yaxis(); ax.set_xlabel("Count"); style_ax(ax)
    save_fig(pdf, fig)
    ...
    fig.suptitle("Commission & Tax Transparency – What Drivers Believe", fontsize=15, fontweight="bold", y=0.97)
    for ax, col, color, label in [
        ...
        data = short[col].dropna(); vc = data.value_counts().head(10); total = len(data)
        ax.barh(vc.index[::-1], vc.values[::-1], color=color, edgecolor="white")
        for i,(k,v) in enumerate(zip(vc.index[::-1], vc.values[::-1])):
            ax.text(v+1, i, f"{v} ({v/total*100:.0f}%)", va="center", fontsize=8)
        ax.set_title(f"{label}  (n={total})", fontsize=10); style_ax(ax)
    save_fig(pdf, fig)
    ...
    # ================================================================
    fig, axes = plt.subplots(1, 2, figsize=(14, 6), facecolor=BG_COLOR)
    fig.suptitle("Unpaid by Passenger – Incident Rate & Compensation", fontsize=15, fontweight="bold", y=0.99)
    ax = axes[0]
    platforms = ["Snapp","Tapsi"]
    followup_cols = ["snapp_unpaid_by_passenger_followup","tapsi_unpaid_by_passenger_followup"]
    ...
    for pi, (platform, col, color) in enumerate(zip(platforms, followup_cols, [SNAPP_COLOR,TAPSI_COLOR])):
        data = short[col].dropna(); total = len(data)
        vals = [len(data[data==o]) for o in outcomes]
        for oi,(o,v) in enumerate(zip(outcomes,vals)):
            ax.bar(oi + pi*(0.35+0.02) - 0.22, v, 0.35, color=outcome_colors[oi], edgecolor="white")
            ax.text(oi + pi*(0.35+0.02) - 0.22, v+5, f"{v/total*100:.0f}%", ha="center", fontsize=8)
    ax.set_xticks(np.arange(3)+0.11)
    ax.set_xticklabels(["No Followup","Followed – Not\nResolved","Followed –\nResolved"], fontsize=9)
    ax.set_ylabel("Count"); ax.set_title("Unpaid Fare Followup", fontsize=11)
    snapp_p = mpatches.Patch(color=SNAPP_COLOR, label="Snapp")
    tapsi_p = mpatches.Patch(color=TAPSI_COLOR, label="Tapsi")
    ax.legend(handles=[snapp_p,tapsi_p], frameon=False, fontsize=9); style_ax(ax)
    ax = axes[1]
    comp_cats = ["Yes - all of it","Yes - Part of it","No compensation"]
    comp_colors = ["#66BB6A","#FFA726","#EF5350"]
    x = np.arange(len(comp_cats)); w = 0.35
    for pi,(platform,(col,pcolor)) in enumerate([("Snapp",("snapp_compensate_unpaid_by_passenger",SNAPP_COLOR)),
        ...
        ax.bar(x+offset, vals, w, color=pcolor, edgecolor="white", label=platform, alpha=0.85)
        for xi,v in enumerate(vals): ax.text(xi+offset, v+1, f"{v/total*100:.0f}%", ha="center", fontsize=8)
    ...
    fig.suptitle("Customer Support Channel Usage – Snapp vs Tapsi", fontsize=15, fontweight="bold", y=0.99)
    for ax, col, color, label in [(axes[0],"snapp_customer_support",SNAPP_COLOR,"Snapp"),
                                   (axes[1],"tapsi_customer_support",TAPSI_COLOR,"Tapsi")]:
        data = short[col].dropna().value_counts(); total = data.sum()
    ...
    fig.suptitle("Customer Support Quality Deep Dive – Snapp vs Tapsi", fontsize=15, fontweight="bold", y=0.99)
    ax = axes[0]
    ...
    for pi,(col,color,label) in enumerate([("snapp_CS_solved",SNAPP_COLOR,"Snapp"),
        ...
        x = np.arange(len(solve_cats)); w = 0.35
        offset = (pi-0.5)*(w+0.02)
        ax.bar(x+offset, vals, w, color=color, edgecolor="white", label=label, alpha=0.85)
        for xi,v in enumerate(vals): ax.text(xi+offset, v+1, f"{v/total*100:.0f}%", ha="center", fontsize=8)
    ax.set_xticks(x); ax.set_xticklabels(solve_cats, fontsize=10)
    ax.set_title("Issue Resolution Rate", fontsize=11); ax.legend(frameon=False, fontsize=9); style_ax(ax)
    ax = axes[1]
    ...
    x = np.arange(len(cs_dims)); w = 0.35
    ax.bar(x-w/2, snapp_cs_means, w, color=SNAPP_COLOR, label="Snapp", edgecolor="white")
    ax.bar(x+w/2, tapsi_cs_means, w, color=TAPSI_COLOR, label="Tapsi", edgecolor="white")
    for xi,(sv,tv) in enumerate(zip(snapp_cs_means,tapsi_cs_means)):
        ax.text(xi-w/2, sv+0.05, f"{sv:.2f}", ha="center", fontsize=7.5, fontweight="bold")
        ax.text(xi+w/2, tv+0.05, f"{tv:.2f}", ha="center", fontsize=7.5, fontweight="bold")
    ax.set_xticks(x); ax.set_xticklabels(cs_dims, fontsize=9, rotation=10)
    ax.set_ylim(0, 5.5); ax.set_title("CS Satisfaction Sub-scores (1–5)", fontsize=11)
    ax.legend(frameon=False, fontsize=9); style_ax(ax)
    ax = axes[2]
    reasons_s = short["snapp_CS_satisfaction_important_reason"].dropna().value_counts().head(5)
    ax.barh(reasons_s.index[::-1], reasons_s.values[::-1], color=SNAPP_COLOR, edgecolor="white")
    total_s = reasons_s.sum()
    for i,v in enumerate(reasons_s.values[::-1]):
        ax.text(v+1, i, f"{v} ({v/total_s*100:.0f}%)", va="center", fontsize=9)
    ax.set_title("Snapp CS Dissatisfaction\nMain Reason (top 5)", fontsize=11); style_ax(ax)
    save_fig(pdf, fig)
    ...
    # ================================================================
    fig, axes = plt.subplots(1, 2, figsize=(14, 6), facecolor=BG_COLOR)
    fig.suptitle("Collaboration Reasons – Why Drivers Chose Each Platform", fontsize=15, fontweight="bold", y=0.99)
    for ax, col, color, label in [(axes[0],"snapp_collab_reason",SNAPP_COLOR,"Snapp"),
        ...
        vc = data.value_counts().head(10); total = data.notna().sum()
        ax.barh(vc.index[::-1], vc.values[::-1], color=color, edgecolor="white")
        for i,v in enumerate(vc.values[::-1]):
            ax.text(v+1, i, f"{v} ({v/total*100:.1f}%)", va="center", fontsize=9)
        ax.set_title(f"{label}  (n={total})", fontsize=11); style_ax(ax)
    save_fig(pdf, fig)
    ...
    # ================================================================
    fig, axes = plt.subplots(1, 2, figsize=(12, 5), facecolor=BG_COLOR)
    fig.suptitle("Which Service Yields Better Income? (Driver Perception)", fontsize=15, fontweight="bold", y=0.99)
    for ax, col, color, label in [(axes[0],"snapp_better_income",SNAPP_COLOR,"Snapp Drivers' View"),
        ...
        data = short[col].dropna().value_counts(); total = data.sum()
        ax.barh(data.index, data.values, color=color, edgecolor="white")
        for i,v in enumerate(data.values): ax.text(v+1, i, f"{v} ({v/total*100:.0f}%)", va="center", fontsize=9)
        ax.set_title(f"{label}  (n={total})", fontsize=11); style_ax(ax)
    save_fig(pdf, fig)
    ...
    # ================================================================
    fig, axes = plt.subplots(1, 3, figsize=(16, 5), facecolor=BG_COLOR, sharey=True)
    fig.suptitle("Avg Satisfaction by Year-Week: Snapp vs Tapsi", fontsize=15, fontweight="bold", y=0.99)
    for ax, (scol, tcol, label) in zip(axes, SAT_PAIRS):
        weekly_sat = short.groupby("yearweek").agg(snapp=(scol,"mean"), tapsi=(tcol,"mean"))
        ax.plot(weekly_sat.index.astype(str), weekly_sat["snapp"], marker="o", color=SNAPP_COLOR, linewidth=2, label="Snapp")
        ax.plot(weekly_sat.index.astype(str), weekly_sat["tapsi"], marker="s", color=TAPSI_COLOR, linewidth=2, label="Tapsi")
        ...
        ax.set_title(label, fontsize=11); ax.set_ylim(0, 5.5); ax.set_xlabel("Year-Week")
        ax.legend(frameon=False, fontsize=8); ax.tick_params(axis="x", rotation=45); style_ax(ax)
    axes[0].set_ylabel("Mean Satisfaction (1–5)")
    save_fig(pdf, fig)
    ...
    # ================================================================
    plot_sat_by_group(pdf, short, "city", "City (Top 10)", top_n=10, min_group_size=20)
    ...
    # ================================================================
    valid_tenure = [t for t in TENURE_ORDER if t in short["snapp_age"].unique()]
    ...
    ax.set_ylim(0, 5.5); ax.set_ylabel("Mean Satisfaction (1–5)")
    ax.set_xlabel("Snapp Tenure"); ax.set_title("Snapp Satisfaction by Tenure", fontsize=11)
    ax.legend(frameon=False, fontsize=9); style_ax(ax)
    ...
    ax.set_ylim(0, 10); ax.set_ylabel("Avg Recommendation Score (0–10)")
    ax.set_xlabel("Snapp Tenure"); ax.set_title("Recommendation by Tenure", fontsize=11)
    ax.legend(frameon=False, fontsize=9); style_ax(ax)
    save_fig(pdf, fig)
    ...
    active_grps = [a for a in active_order if a in df_active["active_time"].unique()]
    ...
    fig.suptitle("Satisfaction & Recommendation by Driver Engagement Level",
                 fontsize=15, fontweight="bold", y=0.99)
    x = np.arange(len(active_grps)); w = 0.35
    xlabels = [active_labels_display.get(a, a) for a in active_grps]

    ax = axes[0]
    ax.bar(x - w/2, active_sat["snapp_overall"], w, color=SNAPP_COLOR, label="Snapp", edgecolor="white")
    ax.bar(x + w/2, active_sat["tapsi_overall"], w, color=TAPSI_COLOR, label="Tapsi", edgecolor="white")
    ax.set_xticks(x); ax.set_xticklabels(xlabels, rotation=15, ha="right")
    ax.set_ylim(0, 5.5); ax.set_title("Overall Satisfaction", fontsize=11)
    ax.legend(frameon=False, fontsize=9); style_ax(ax)
    ...
    ax.set_ylim(0, 10); ax.set_title("Recommendation Score (0–10)", fontsize=11)
    ax.tick_params(axis="x", rotation=15); ax.legend(frameon=False, fontsize=9); style_ax(ax)
    save_fig(pdf, fig)
    ...
    ax.barh(jobs, job_sat["snapp_overall"], color=SNAPP_COLOR, alpha=0.85, label="Snapp Overall")
    ax.barh(jobs, job_sat["tapsi_overall"], color=TAPSI_COLOR, alpha=0.55, label="Tapsi Overall", left=0)
    ...
    ax = axes[0, 1]
    offer_data = short["tapsi_carpooling_gotoffer_accepted"].dropna().value_counts()
    offer_colors = {"No": LGREY, "got offer - rejected": "#FF6D00", "got offer - accepted": "#66BB6A"}
    ...
    for i, v in enumerate(offer_data.values):
        ax.text(v + 5, i, f"{v} ({v/total*100:.0f}%)", va="center", fontsize=9)
    ax.set_title("Carpooling Offer Outcome", fontsize=11); style_ax(ax)
    ...
    r_vals   = [wide[c].sum() if c in wide.columns else 0 for c in carp_refusal.values()]
    ...
    ax.set_title("Carpooling Refusal Reasons (wide_survey)", fontsize=11); style_ax(ax)
    ...
    for xi, v in zip(sat_counts.index.astype(int).astype(str), sat_counts.values):
        ax.text(xi, v + 1, f"{v} ({v/total*100:.0f}%)", ha="center", fontsize=9)
    ax.set_title(f"Carpooling Satisfaction (1–5, mean={carp_sat.mean():.2f})", fontsize=11)
    ax.set_xlabel("Rating"); style_ax(ax)
    save_fig(pdf, fig)
    ...
    ax.set_title("Snapp EcoPlus Adoption Funnel", fontsize=11); style_ax(ax)
    ...
    for i, (k, v) in enumerate(mw.items()):
        ax.text(i, v + 10, f"{v:,} ({v/total_mw*100:.0f}%)", ha="center", fontsize=9)
    ax.set_title("Tapsi Magical Window Awareness", fontsize=11); style_ax(ax)
    save_fig(pdf, fig)
    ...
    for i, v in enumerate(feeling.values):
        ax.text(v + 3, i, f"{v} ({v/total_f*100:.0f}%)", va="center", fontsize=9)
    ax.set_title("Comfort Level When Working\nAround Other People", fontsize=11)
    ax.set_xlabel("Count"); style_ax(ax)
    ...
    for i, v in enumerate(reason.values):
        ax.text(v + 1, i, f"{v} ({v/total_r*100:.0f}%)", va="center", fontsize=9)
    ...
    for ax, col, color, label in [
        ...
        for i, v in enumerate(data.values):
            ax.text(v + 1, i, f"{v} ({v/total*100:.0f}%)", va="center", fontsize=9)
        ax.set_title(label, fontsize=11); style_ax(ax)
    save_fig(pdf, fig)
    ...
    for ax, ride_col, cf_col, color, label in [
        ...
        ax.set_xlabel("Total Rides"); ax.set_title(f"{label}  (n={mask.sum()})", fontsize=11)
        ax.legend(frameon=False, fontsize=9); style_ax(ax)
    ...
    ax.set_xlabel("Responses"); style_ax(ax)
    save_fig(pdf, fig)
    ...
    cities = city_sat.index.tolist(); x = np.arange(len(cities)); w = 0.35
    ax = axes[0]
    ax.barh(x - w/2, city_sat["snapp_sat"], w, color=SNAPP_COLOR, label="Snapp", edgecolor="white")
    ax.barh(x + w/2, city_sat["tapsi_sat"], w, color=TAPSI_COLOR, label="Tapsi", edgecolor="white")
    ax.set_yticks(x); ax.set_yticklabels(cities)
    ax.set_xlim(0, 6); ax.set_xlabel("Mean Satisfaction (1–5)")
    ax.set_title("Overall Satisfaction", fontsize=11)
    ax.legend(frameon=False, fontsize=9); style_ax(ax)

    ax = axes[1]
    ax.barh(x - w/2, city_sat["snapp_rec"], w, color=SNAPP_COLOR, label="Snapp", edgecolor="white")
    ax.barh(x + w/2, city_sat["tapsi_rec"], w, color=TAPSI_COLOR, label="Tapsi", edgecolor="white")
    ax.set_yticks(x); ax.set_yticklabels(cities)
    ...
    for (ax, col, color, label) in [
        ...
        ax.barh(data.index[::-1], data.values[::-1], color=color, edgecolor="white")
        for i, v in enumerate(data.values[::-1]):
            ax.text(v + 2, i, f"{v} ({v/total*100:.0f}%)", va="center", fontsize=9)
        ax.set_title(label, fontsize=10); style_ax(ax)
    save_fig(pdf, fig)
    ...
    for ax, question, color, label in [
        (axes[0], "Tapsi Incentive Unsatisfaction", TAPSI_COLOR, "Incentive Unsatisfaction"),
        (axes[1], "Tapsi Carpooling Refusion",       TAPSI_COLOR, "Carpooling Refusion"),
    ]:
        qdata = long[long["question"] == question]
        if len(qdata) == 0:
            ax.set_title(f"{label} (no data)", fontsize=11); style_ax(ax); continue
        ...
        ax.barh(vc.index, vc.values, color=color, edgecolor="white")
        for i, (k, v) in enumerate(vc.items()):
            ax.text(v + 0.3, i, f"{v} ({v/total*100:.0f}%)", va="center", fontsize=9)
        ax.set_title(f"{label}  (n={total})", fontsize=11); style_ax(ax)
        ax.set_xlabel("Count")
    save_fig(pdf, fig)
    ...
    for q, platform, color in [
        ...
        if len(qdata) == 0:
            continue
        pivot = qdata.groupby(["driver_type", "answer"]).size().unstack(fill_value=0)
        answers = pivot.columns.tolist(); groups = pivot.index.tolist()
        fig, ax = new_fig(f"{platform} Customer Support by Driver Type", figsize=(14, 6))
        y = np.arange(len(answers)); n_g = len(groups); total_w = 0.7; w = total_w / n_g
        ...
        ax.set_yticks(y); ax.set_yticklabels(answers)
        ax.legend(frameon=False, fontsize=9); ax.set_xlabel("Count"); style_ax(ax)
        save_fig(pdf, fig)
    ...
    fig.suptitle("Joint vs Snapp Exclusive – Key Metric Comparison",
                 fontsize=15, fontweight="bold", y=0.99)
    groups_ = metrics_by_dt.index.tolist(); x = np.arange(len(groups_)); w = 0.35

    ax = axes[0]
    ax.bar(x - w/2, metrics_by_dt["snapp_sat"], w, color=SNAPP_COLOR, label="Snapp Sat.", edgecolor="white")
    ax.bar(x + w/2, metrics_by_dt["tapsi_sat"], w, color=TAPSI_COLOR, label="Tapsi Sat.", edgecolor="white")
    ax.set_xticks(x); ax.set_xticklabels(groups_); ax.set_ylim(0, 5.5)
    for xi, (sv, tv) in enumerate(zip(metrics_by_dt["snapp_sat"], metrics_by_dt["tapsi_sat"])):
        ax.text(xi - w/2, sv + 0.05, f"{sv:.2f}", ha="center", fontsize=9, fontweight="bold")
        ax.text(xi + w/2, tv + 0.05, f"{tv:.2f}", ha="center", fontsize=9, fontweight="bold")
    ax.set_title("Overall Satisfaction", fontsize=11); ax.legend(frameon=False, fontsize=9); style_ax(ax)

    ax = axes[1]
    ax.bar(x - w/2, metrics_by_dt["snapp_ride"], w, color=SNAPP_COLOR, label="Snapp Rides", edgecolor="white")
    ax.bar(x + w/2, metrics_by_dt["tapsi_ride"], w, color=TAPSI_COLOR, label="Tapsi Rides", edgecolor="white")
    ax.set_xticks(x); ax.set_xticklabels(groups_)
    for xi, (sv, tv) in enumerate(zip(metrics_by_dt["snapp_ride"], metrics_by_dt["tapsi_ride"])):
        ax.text(xi - w/2, sv + 0.3, f"{sv:.0f}", ha="center", fontsize=9, fontweight="bold")
        ax.text(xi + w/2, tv + 0.3, f"{tv:.0f}", ha="center", fontsize=9, fontweight="bold")
    ax.set_title("Avg Weekly Rides", fontsize=11); ax.legend(frameon=False, fontsize=9); style_ax(ax)

    ax = axes[2]
    ax.bar(x - w/2, metrics_by_dt["snapp_inc"] / 1e6, w, color=SNAPP_COLOR, label="Snapp Incentive", edgecolor="white")
    ax.bar(x + w/2, metrics_by_dt["tapsi_inc"] / 1e6, w, color=TAPSI_COLOR, label="Tapsi Incentive", edgecolor="white")
    ax.set_xticks(x); ax.set_xticklabels(groups_)
    for xi, (sv, tv) in enumerate(zip(metrics_by_dt["snapp_inc"] / 1e6, metrics_by_dt["tapsi_inc"] / 1e6)):
        ax.text(xi - w/2, sv + 0.05, f"{sv:.1f}M", ha="center", fontsize=9, fontweight="bold")
        ax.text(xi + w/2, tv + 0.05, f"{tv:.1f}M", ha="center", fontsize=9, fontweight="bold")
    ax.set_title("Avg Incentive (M Rials)", fontsize=11); ax.legend(frameon=False, fontsize=9); style_ax(ax)
    save_fig(pdf, fig)
    ...
    bars = ax.bar(funnel_labels, funnel_vals, color=funnel_colors, edgecolor="white")
    ...
    ax.set_ylabel("Count"); style_ax(ax)
    ...
    bars2 = ax.bar(funnel_labels, t_funnel_vals, color=funnel_colors, edgecolor="white")
    ...
    for ax, col, color, label in [
        ...
        if col not in short.columns:
            ax.set_title(f"{label} (no data)"); style_ax(ax); continue
        data = short[col].dropna()
        ...
        ax.set_xlabel("Duration"); ax.set_ylabel("Count"); style_ax(ax)
    save_fig(pdf, fig)
    ...
    if inact_col in short.columns:
        ...
        for i, (k, v) in enumerate(zip(vc.index[::-1], vc.values[::-1])):
            ax.text(v + 20, i, f"{v:,} ({v/total*100:.0f}%)", va="center", fontsize=9)
        ax.set_xlabel("Count")
        ax.set_title(f"(n={total:,}) — orange = recently active, grey = long dormant", fontsize=10)
    else:
    ...
    nps_pairs = [
        (axes[0, 0], "snapp_refer_others", SNAPP_COLOR, "Snapp APP NPS\n(recommend the SnappDriver app)"),
        (axes[0, 1], "snapp_recommend",    SNAPP_COLOR, "Snapp PLATFORM NPS\n(recommend driving on Snapp)"),
        (axes[1, 0], "tapsi_refer_others", TAPSI_COLOR, "Tapsi APP NPS\n(recommend the Tapsi Driver app)"),
        (axes[1, 1], "tapsi_recommend",    TAPSI_COLOR, "Tapsi PLATFORM NPS\n(recommend driving on Tapsi)"),
    ]

    for ax, col, color, label in nps_pairs:
        if col not in short.columns:
            ax.set_title(f"{label}\n(no data)", fontsize=9); style_ax(ax); continue
        data = short[col].dropna()
        nps = nps_score(data)
        ...
        bar_clr = ["#EF5350" if s <= 6 else ("#B0BEC5" if s <= 8 else "#66BB6A")
                   for s in vc.index]
        ax.bar(vc.index.astype(str), vc.values, color=bar_clr, edgecolor="white")
        ...
        ax.set_title(f"{label}\nNPS={nps:+.0f}  |  P={promoters/total*100:.0f}% D={detractors/total*100:.0f}% (n={total:,})",
                     fontsize=9)
        ax.set_xlabel("Score (0–10)"); ax.set_ylabel("Count"); style_ax(ax)
    save_fig(pdf, fig)
    ...
    for ax, comm_col, sat_col, color, label in [
        (axes[0], "snapp_comm_info", "snapp_overall_satisfaction", SNAPP_COLOR, "Snapp"),
        (axes[1], "tapsi_comm_info", "tapsi_overall_satisfaction", TAPSI_COLOR, "Tapsi"),
    ]:
        ...
        for i, (k, v) in enumerate(comm_groups.items()):
            n = n_per_group[k]
            ax.text(v + 0.02, i, f"{v:.2f}  (n={n:,})", va="center", fontsize=8)
        ax.set_xlim(0, 5.5)
    ...
    followup_pairs = [
        ("snapp_satisfaction_followup_overall", "tapsi_satisfaction_followup_overall", "Overall Satisfaction with Follow-up"),
        ("snapp_satisfaction_followup_time",    "tapsi_satisfaction_followup_time",    "Satisfaction with Time-to-Resolve"),
    ]

    for ax, (scol, tcol, label) in zip(axes, followup_pairs):
        s_data = short[scol].dropna() if scol in short.columns else pd.Series(dtype=float)
        t_data = short[tcol].dropna() if tcol in short.columns else pd.Series(dtype=float)
        ratings = [1, 2, 3, 4, 5]
        ...
        ax.bar(x - w/2, s_counts, w, color=SNAPP_COLOR, label=f"Snapp (n={len(s_data):,})", edgecolor="white")
        ax.bar(x + w/2, t_counts, w, color=TAPSI_COLOR, label=f"Tapsi (n={len(t_data):,})", edgecolor="white")
        s_mean = s_data.mean() if len(s_data) else 0
        t_mean = t_data.mean() if len(t_data) else 0
        ax.set_xticks(x); ax.set_xticklabels([str(r) for r in ratings])
    ...
    for ax, col, color, label in [
        ...
        for i, (k, v) in enumerate(vc.items()):
            ax.text(i, v + 20, f"{v:,}\n({v/total*100:.0f}%)", ha="center", fontsize=9)
        ax.set_title(f"{label} Trip Length Preference  (n={total:,})", fontsize=11)
        ax.set_xlabel("Trip Type"); ax.set_ylabel("Count"); style_ax(ax)
    save_fig(pdf, fig)
    ...
    # ================================================================
    snapp_nav_order = ["Neshan", "Balad", "Google Map", "Waze", "No Navigation App"]
    tapsi_nav_order = ["Neshan", "Balad", "In-App Navigation", "No Navigation App"]
    ...
    for ax, col, nav_order, color, label in [
        (axes[0], "snapp_last_trip_navigation", snapp_nav_order, SNAPP_COLOR, "Snapp"),
        (axes[1], "tapsi_navigation_type",      tapsi_nav_order, TAPSI_COLOR, "Tapsi"),
    ]:
        if col not in short.columns:
            ax.set_title(f"{label} (no data)"); style_ax(ax); continue
        data = short[col].dropna()
        ...
        for k in vc.index:
            if "No" in k:
                ...
                bar_colors_nav.append(TAPSI_COLOR if color == TAPSI_COLOR else ACCENT2)
            else:
                bar_colors_nav.append(color)
        ax.barh(vc.index[::-1], vc.values[::-1],
                color=list(reversed(bar_colors_nav)), edgecolor="white")
        for i, (k, v) in enumerate(zip(vc.index[::-1], vc.values[::-1])):
            ax.text(v + 50, i, f"{v:,} ({v/total*100:.0f}%)", va="center", fontsize=9)
        ax.set_title(f"{label} Navigation in Last Trip  (n={total:,})", fontsize=11)
        ax.set_xlabel("Count"); style_ax(ax)
    save_fig(pdf, fig)
    ...
    for ax, col, color, label in [
        ...
        for i, (k, v) in enumerate(data.items()):
            ax.text(i, v + 20, f"{v:,}\n({v/total*100:.0f}%)", ha="center", fontsize=9)
        ax.set_title(f"{label} Joining/Registration Bonus  (n={total:,})", fontsize=11)
        ax.set_xlabel("Received Bonus?"); ax.set_ylabel("Count"); style_ax(ax)
    save_fig(pdf, fig)
    ...
    nav_items = [
        (axes[0, 0], "tapsi_in_app_navigation_usage",         TAPSI_COLOR, "Used Tapsi In-App Navigation"),
        (axes[0, 1], "tapsi_in_app_navigation_satisfaction",  TAPSI_COLOR, "In-App Navigation Satisfaction (1–5)"),
        (axes[1, 0], "tapsi_offline_navigation_familiar",     "#5C6BC0",   "Familiar with Tapsi Offline Navigation"),
        (axes[1, 1], "tapsi_offline_navigation_usage",        "#5C6BC0",   "Offline Navigation Usage During GPS Issues"),
    ]

    for ax, col, color, label in nav_items:
        if col not in short.columns:
            ax.set_title(f"{label}\n(no data)", fontsize=10); style_ax(ax); continue
        data = short[col].dropna().value_counts()
        total = data.sum()
    ...
    if gps_col in short.columns:
        ...
        for i, (k, v) in enumerate(data.items()):
            ax.text(i, v + 5, f"{v:,}\n({v/total*100:.0f}%)", ha="center", fontsize=9)
        ax.set_title(f"Was Tapsi App Better\nDuring GPS Issues?  (n={total:,})", fontsize=10)
        ax.set_ylabel("Count"); style_ax(ax)
    else:
        ax.set_title("tapsi_gps_better (no data)"); style_ax(ax)
    ...
    if mw_col in short.columns:
        ...
        for i, (k, v) in enumerate(data.items()):
            ax.text(i, v + 10, f"{v:,}\n({v/total*100:.0f}%)", ha="center", fontsize=9)
        ax.set_title(f"Tapsi Magical Window\nAwareness  (n={total:,})", fontsize=10)
        ax.set_ylabel("Count"); style_ax(ax)
    else:
        ax.set_title("tapsi_magical_window (no data)"); style_ax(ax)

    # Panel 3: Tapsi Referral Program (from long survey)
    ax = axes[2]
    tapsi_ref_q = long[long["question"] == "Tapsi Incentive GotBonus"] if "question" in long.columns else pd.DataFrame()
    if len(tapsi_ref_q) > 0:
        ...
        for i, (k, v) in enumerate(vc.items()):
            ax.text(v + 1, i, f"{v} ({v/total*100:.0f}%)", va="center", fontsize=8)
        ax.set_title(f"Tapsi Incentive GotBonus\n(long_survey, n={total:,})", fontsize=10)
        ax.set_xlabel("Count"); style_ax(ax)
    else:
        ax.set_title("Tapsi Incentive GotBonus\n(no data in long_survey)", fontsize=10)
        style_ax(ax)
    save_fig(pdf, fig)
