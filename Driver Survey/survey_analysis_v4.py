"""
Driver Survey – Visual Analysis v4
====================================
Comprehensive data-insight-driven report.
New in v4 (vs v3):
  - Cover page with key KPI summary
  - Fixes Navigation Funnel to use long_survey (wide had only Familiarity)
  - Page: Satisfaction Honeymoon Effect (by Snapp tenure)
  - Page: Satisfaction by Age Group
  - Page: Satisfaction by Active-Time Commitment
  - Page: Satisfaction by Occupation (top jobs)
  - Page: Customer Support Deep Dive (channel, solve rate, sub-scores)
  - Page: Navigation App Ratings (0–10 NPS-style) + In-App Nav comparison
  - Page: GPS Failure Stage Distribution (Snapp vs Tapsi)
  - Page: GPS Glitch Time-of-Day (wide_survey)
  - Page: Unpaid Fares – Incident Rate & Compensation Outcome
  - Page: Commission & Tax Transparency (driver perception)
  - Page: Collaboration Reasons (why they chose each platform)
  - Page: Income Source Preference
  - Page: EcoPlus & Magical Window Adoption
  - Page: Driver Privacy / Participation Feelings
  - Page: Demand & Missed-Demand Analysis
  - Page: City Satisfaction Comparison
  - Page: Tapsi Registration & Referral Funnel
  - Page: Long-survey: Navigation Unsatisfaction & Refusal Reasons
  - Page: Long-survey: Decline Reasons & SnappDriver App Menu
  - Page: Long-survey: Snapp Ecoplus Refusal
  - Improved: Tapsi Carpooling full analysis page
Usage:
    python survey_analysis_v4.py
"""
...
# Sync wide and long to surviving recordIDs
...
# ── Filter weeks with < MIN_WEEK_RESPONSES ───────────────────────────
...
def add_value_labels_h(ax, values, y_positions, total=None, offset=2, fontsize=9):
    """Add count (and optional %) labels to horizontal bars."""
    for i, v in enumerate(values):
        if v > 0:
            label = f"{int(v)}" if total is None else f"{int(v)} ({v/total*100:.0f}%)"
            ax.text(v + offset, y_positions[i], label, va="center", fontsize=fontsize)
...
with PdfPages(OUTPUT_PDF) as pdf:
    ...
    fig.suptitle("Driver Survey – Key Performance Indicators", fontsize=18,
    ...
    insights = [
        ...
        "• Navigation: Neshan rated 7.2/10, Waze 6.4, Balad 6.2, Google Maps 4.7 by drivers",
        "• Joint drivers earn 27% more incentive from Tapsi (6.36M) vs Snapp (5.0M) Rials/week",
        "• GPS issues affect 'All stages' most on Snapp; Tapsi has more 'No problem' responses",
    ]
    ...
    # ================================================================
    # PAGE 3 – DEMOGRAPHICS OVERVIEW (2x2 grid)
    # ================================================================
    ...
    for i, v in enumerate(ag.values):
        ax.text(v + 5, i, f"{v} ({v/len(short)*100:.0f}%)", va="center", fontsize=9)

    ax = axes[0, 1]
    ed = short["edu"].value_counts().sort_index()
    ax.barh(["High School\nor Below", "College+"], ed.values, color=[GREY, ACCENT])
    ...
    # ================================================================
    # PAGE 4 – OCCUPATION BREAKDOWN (top 15 jobs)
    # ================================================================
    ...
    ax.set_title("Note: 'Just in IT' = dedicated ride-hailing driver (IT = Tahvile Toro)", fontsize=9,
                 style="italic", pad=3)
    style_ax(ax)
    save_fig(pdf, fig)
    ...
    weekly_rides = short.groupby("yearweek").agg(
        snapp_ride=("snapp_ride", "mean"), tapsi_ride=("tapsi_ride", "mean"))
    ...
    ax.set_xlabel("Year-Week"); ax.set_ylabel("Avg Rides (midpoint)")
    ...
    for ax, (scol, tcol, label) in zip(axes, SAT_PAIRS):
        ...
        bars = ax.bar(["Snapp", "Tapsi"], [snapp_mean, tapsi_mean],
                      color=[SNAPP_COLOR, TAPSI_COLOR], width=0.5, edgecolor="white")
    ...
    ax.legend(frameon=False, fontsize=10)
    ax.set_xlabel("Year-Week"); ax.set_ylabel("NPS")
    ax.tick_params(axis="x", rotation=45); style_ax(ax)
    save_fig(pdf, fig)
    ...
    # ================================================================
    # PAGE 12 – INCENTIVE UNSATISFACTION REASONS (wide_survey)
    # ================================================================
    unsat_types = {
        "Snapp": {
    ...
    ax.plot(weekly_inc.index.astype(str), weekly_inc["tapsi"] / 1e6,
            marker="s", color=TAPSI_COLOR, linewidth=2.5, label="Tapsi")
    ax.legend(frameon=False, fontsize=10)
    ax.set_xlabel("Year-Week"); ax.set_ylabel("Avg Incentive (Million Rials)")
    ax.tick_params(axis="x", rotation=45); style_ax(ax)
    save_fig(pdf, fig)
    ...
    for ax, col, color, label in [
        ...
        ax.set_xlabel("Months"); style_ax(ax)
    axes[0].set_ylabel("Count")
    save_fig(pdf, fig)

    # ================================================================
    # PAGE 16 – RIDE REFUSAL REASONS (wide_survey)
    # ================================================================
    refusal_labels = [
    ...
    ax.invert_yaxis(); ax.legend(frameon=False, fontsize=10); ax.set_xlabel("Count")
    style_ax(ax)
    save_fig(pdf, fig)

    # ================================================================
    # PAGE 17 – CUSTOMER SUPPORT CATEGORY (wide_survey)
    # ================================================================
    ...
    tapsi_cs_labels_raw = ["Fare", "Cancelling", "Trip Problems", "Petrol",
                           "Technical", "Settlement", "Incentive",
                           "Loc Change", "Drivers Club", "Registration"]
    tapsi_cs = [f"Tapsi Customer Support Category__{l}" for l in tapsi_cs_labels_raw]
    ...
    # Right: distribution for Neshan vs Balad (most used)
    ax = axes[1]
    ...
    # ================================================================
    # PAGE 20 – GPS FAILURE STAGE DISTRIBUTION (Snapp vs Tapsi)
    # ================================================================
    ...
    # ================================================================
    # PAGE 21 – GPS GLITCH TIME OF DAY (wide_survey)
    # ================================================================
    gps_time_cols = {
    ...
    # ================================================================
    # PAGE 22 – GPS GLITCH ACTIONS (wide_survey)
    # ================================================================
    gps_action_map = {
        "Called Passenger":       "GPS Action when Glitch__Called Passenger",
    ...
    for pi, (platform, col, color) in enumerate(zip(platforms, followup_cols, [SNAPP_COLOR, TAPSI_COLOR])):
        ...
        for oi, (o, v) in enumerate(zip(outcomes, vals)):
            ax.bar(oi + pi * (w + 0.02) - 0.22, v, w, color=outcome_colors[oi],
                   edgecolor="white", label=f"{platform}: {o}" if pi == 0 else None)
            ax.text(oi + pi * (w + 0.02) - 0.22, v + 5, f"{v/total*100:.0f}%",
    ...
    snapp_patch = mpatches.Patch(color=SNAPP_COLOR, label="Snapp")
    tapsi_patch = mpatches.Patch(color=TAPSI_COLOR, label="Tapsi")
    ax.legend(handles=[snapp_patch, tapsi_patch], frameon=False, fontsize=9)
    style_ax(ax)

    # Right: compensation outcome
    ax = axes[1]
    comp_map = {
    ...
    for pi, (platform, (col, pcolor)) in enumerate(comp_map.items()):
        data = short[col].dropna()
        total = len(data)
        vals = [len(data[data == c]) for c in comp_cats]
        offset = (pi - 0.5) * (w + 0.02)
        bars = ax.bar(x + offset, vals, w, color=pcolor, edgecolor="white",
                      label=platform, alpha=0.85)
        for xi, v in enumerate(vals):
            ax.text(xi + offset, v + 1, f"{v/total*100:.0f}%", ha="center", fontsize=8)
    ...
    # ================================================================
    # PAGE 26 – CUSTOMER SUPPORT DEEP DIVE (solve rate + sub-scores)
    # ================================================================
    ...
    # Important reason for CS dissatisfaction
    ax = axes[2]
    reasons_s = short["snapp_CS_satisfaction_important_reason"].dropna().value_counts().head(5)
    reasons_t = short["tapsi_CS_satisfaction_important_reason"].dropna().value_counts().head(5)
    # Show Snapp (most complaints since lower sat)
    ...
    # ================================================================
    # PAGE 27 – COLLABORATION REASONS (why drivers chose each platform)
    # ================================================================
    fig, axes = plt.subplots(1, 2, figsize=(14, 6), facecolor=BG_COLOR)
    ...
    for ax, col, color, label in [
        ...
        data = short[col].dropna()
        # filter out Farsi / custom entries for readability
    ...
    # ================================================================
    fig, axes = plt.subplots(1, 2, figsize=(12, 5), facecolor=BG_COLOR)
    fig.suptitle("Which Platform Yields Better Income? (Driver Perception)",
                 fontsize=15, fontweight="bold", y=0.99)
    ...
    # ================================================================
    # PAGE 29 – SATISFACTION BY YEARWEEK (line charts)
    # ================================================================
    ...
    for ax, (scol, tcol, label) in zip(axes, SAT_PAIRS):
        ...
        ax.set_xlabel("Year-Week"); ax.legend(frameon=False, fontsize=8)
        ax.tick_params(axis="x", rotation=45); style_ax(ax)
    axes[0].set_ylabel("Mean Satisfaction (1–5)")
    save_fig(pdf, fig)
